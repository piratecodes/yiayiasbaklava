  ...
  return (
    <nav className={`sticky top-0 left-0 py-2 w-full z-50 transform transition-normal ease-initial ${pathName === "/" ? scrollPosition > 50 ? 'bg-sky-100 shadow-md' : 'bg-transparent' : 'bg-sky-100 shadow-md'}`}>
      ...
      <aside ref={ref3} className='absolute inset-y-0 flex flex-col w-full h-screen z-30 md:w-4/6 lg:w-1/4 bg-sky-500/95 md:rounded-r-4xl transition ease-linear duration-300 -translate-x-full'>
        ...
        <ol className='mt-5 space-y-5 text-white font-bold text-3xl w-11/12 mx-auto'>
          ...
          <li><Link href="/favourite" onClick={handleMenuClick}>Favourite</Link></li>
          ...
          <li className="flex flex-row space-x-1.5 lg:hidden">
            ...
            <Link
              href="/order-now"
              className={`inline-flex items-center px-4 py-2 text-sm font-medium hover:text-gray-60 rounded-4xl transform transition-colors delay-300 ease-in ${scrollPosition > 50 ? 'bg-sky-500/75 text-white' : 'bg-transparent text-gray-800'}`}
            >
              <svg className="w-4 h-4 mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 544 512">
                <path d="M527.79 288H290.5l158.03 158.03c6.04 6.04 15.98 6.53 22.19.68 38.7-36.46 65.32-85.61 73.13-140.86 1.34-9.46-6.51-17.85-16.06-17.85zm-15.83-64.8C503.72 103.74 408.26 8.28 288.8.04 279.68-.59 272 7.1 272 16.24V240h223.77c9.14 0 16.82-7.68 16.19-16.8zM224 288V50.71c0-9.55-8.39-17.4-17.84-16.06C86.99 51.49-4.1 155.6.14 280.37 4.5 408.51 114.83 513.59 243.03 511.98c50.4-.63 96.97-16.87 135.26-44.03 7.9-5.6 8.42-17.23 1.57-24.08L224 288z" />
              </svg>
