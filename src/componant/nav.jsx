...
export default function Nav() {
  ...
  const threshold = 50; // Scroll position to trigger color change
  ...
  return (
    <nav className={`sticky top-0 left-0 w-full z-50 transform transition-colors delay-500 ease-in ${scrollPosition > threshold ? 'bg-cyan-500/25 backdrop-blur-lg' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            <Link
              href="/order-now"
              className={`inline-flex items-center px-4 py-2 text-sm font-medium hover:text-gray-60 rounded-4xl transform transition-colors delay-300 ease-in ${scrollPosition > threshold ? 'bg-cyan-900/75 text-white' : 'bg-transparent text-gray-800'}`}
            >
      ...
      <aside ref={ref3} className='absolute inset-y-0 flex flex-col w-full h-screen z-30 md:w-4/6 lg:w-1/4 bg-cyan-200/90 rounded-r-4xl transition ease-linear duration-300 -translate-x-full'>
        <div className='flex justify-between h-10 w-5/6 my-4 mx-auto'>
          <span><label className='font-bold text-3xl' style={{ fontFamily: "'Hind Siliguri', sans-serif" }}> Sign In</label> </span>
          <span><button aria-label="Close Cart" title="Close Cart" className="focus:shadow-outline -mr-1 rounded-full p-2 transition duration-200 hover:bg-deep-purple-50 hover:bg-opacity-30 bg-cyan-400" onClick={() => toogleMenu()}><GrClose className="w-5 text-lg text-black" /></button></span>
        </div>
        <hr className='border border-gray-400 my-1 w-11/12 mx-auto ' />
        <ol className='mt-5 space-y-5 font-bold text-5xl w-11/12 mx-auto'>
          <li><Link href="/" onClick={handleMenuClick}>Home</Link></li>
          <li><Link href="/order" onClick={handleMenuClick}>Order</Link></li>
          <li><Link href="#" onClick={handleMenuClick}>Location</Link></li>
