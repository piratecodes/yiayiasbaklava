                ...
                <section className="p-5 w-full max-w-6xl border-2 border-sky-500 rounded-lg space-y-2.5">
                    {vote.data.map((i, j)=>{
                        return <div key={j} className={`p-3.5 flex flex-col items-center border border-gray-200 rounded-lg shadow-sm md:flex-row ${i.is_voted == 1 ? "bg-sky-500" : "bg-white"}`}>
                            ...
                            <figure className='relative w-32 h-32 md:h-48 md:w-48 md:rounded-none md:rounded-s-lg overflow-hidden'><Image fill objectFit='contain' src={i.product_image} alt={i.product_name} draggable="false" /></figure>
                            <div className="flex flex-col justify-between p-4 leading-normal">
                                <p className={`mb-3 font-semibold ${i.is_voted == 0 ? "text-sky-500" : "text-white"} `}>Vote Count: {i.vote_count}</p>
                            ...
                            <button type="button" onClick={(e)=> handleVote(e, i.product_id)} className="w-full mx-auto md:w-fit md:mx-0 md:ml-auto px-4 py-2 text-md font-medium text-center inline-flex items-center text-white bg-sky-300 rounded-lg hover:bg-sky-500 cursor-pointer"> <BsFileArrowUpFill /> Vote </button>
                        </div>
                    })}
                </section>
            </main>
